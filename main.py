...
PROMPT_DEFINE_STAGES = '''You are a technology analyst creating a clear and logical report structure.
The user wants a report on: **{technologies}**. The report mode is: **{report_mode}**.

- If `report_mode` is "single", create a standard report structure. Example: ["Introduction", "Core Architecture", "Performance", "Developer Experience", "Ecosystem", "Conclusion"]
- If `report_mode` is "comparison", create a structure that directly compares the technologies. Use clear, direct titles. **Do not repeat "Comparative Analysis" in every title.**

**Example for "comparison" of Next.js vs Remix:**
["Introduction", "Core Architecture and Philosophy", "Rendering Strategies and Performance", "Data Fetching and Mutations", "Developer Experience and Tooling", "Conclusion", "Final Assessment"]
...
- Write the main body of the section first, without any inline citations.
- After the main body, add a `### Sources` subsection.
- Under the `Sources` subsection, list all the `source_url`s you used to write the section in a bulleted list.
...
- If you find conflicting information, present both sides and cite their respective sources in the `Sources` subsection.
- Do NOT include the main section title (e.g., "## {stage_name}") in your response.
...
PROMPT_FINAL_REVIEW = '''You are a Chief Technology Officer providing a final verdict on a technology comparison.
Read the entire report draft below. Your goal is to provide a sharp, conclusive "Final Assessment" that a senior engineering leader would write.

**Your Task:**
1.  Briefly identify any remaining contradictions, weak justifications, or critical information gaps in the report.
2.  Provide a direct, executive summary.
3.  Deliver a clear verdict. State which technology is the recommended choice for specific, distinct use cases (e.g., "For content-heavy marketing sites, Technology A is the clear winner due to... However, for complex, data-intensive web applications, Technology B is the superior choice because...").
4.  Structure your response as a single, concise markdown block, starting with the `## Final Assessment` heading.
...
def get_llm(model_type: Literal["pro", "flash"]) -> ChatGoogleGenerativeAI:
    ...
    return ChatGoogleGenerativeAI(model=model_name, temperature=0.0, timeout=120)
...
def generate_report_iteratively(state: ResearchState) -> ResearchState:
    ...
    # Determine which technologies were actually discussed in the ranked sources
    all_discussed_technologies = set()
    if ranked_sources:
        for sources_in_stage in ranked_sources.values():
            for source in sources_in_stage:
                for tech in source.get('discussed_technologies', []):
                    all_discussed_technologies.add(tech)

    # Filter the original technologies list to only include those found in sources
    final_technologies_to_discuss = [
        tech for tech in technologies if tech in all_discussed_technologies
    ]

    if not final_technologies_to_discuss:
        logger.warning("No relevant information found for any of the requested technologies. Ending report generation.")
        state['report_draft'] = "No information found for the requested technologies in the provided sources."
        return state

    logger.info(f"Original technologies requested: {technologies}")
    logger.info(f"Generating report for technologies found in sources: {final_technologies_to_discuss}")
    ...
    for stage in report_stages:
        ...
        if stage.lower() not in ["introduction", "conclusion", "final assessment"]:
            query = f"Information about {stage} for {' vs '.join(final_technologies_to_discuss)}"
        ...
        prompt = PROMPT_WRITE_SECTION.format(
            stage_name=stage,
            technologies=", ".join(final_technologies_to_discuss),
        ...
        try:
            ...
            logger.error(f"Error generating section '{stage}': {e}")
            report_draft += f"## {stage}\n\n_Error: Could not generate this section due to an unexpected error._\n\n"
